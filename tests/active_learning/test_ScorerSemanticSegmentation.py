...
class TestScorerSemanticSegmentation(unittest.TestCase):
    def setUp(self):
        ...
        self.dummy_data_width_1 = self.dummy_data_width_1.reshape(
            self.N, 1, self.H, self.C
        )
        ...
        self.dummy_data_height_1 = self.dummy_data_height_1.reshape(
            self.N, self.W, 1, self.C
        )

        self.dummy_data_width_height_1 = np.random.randn(self.N, self.C)
        self.dummy_data_width_height_1 /= np.sum(
            self.dummy_data_width_height_1, axis=-1
        )[:, None]
        self.dummy_data_width_height_1 = self.dummy_data_width_height_1.reshape(
            self.N, 1, 1, self.C
        )

        self.dummy_data_classes_1 = np.random.randn(self.N * self.W * self.H, 1)
        self.dummy_data_classes_1 = self.dummy_data_classes_1.reshape(
            self.N, self.W, self.H, 1
        )
