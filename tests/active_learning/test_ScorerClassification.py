...
class TestScorerClassification(unittest.TestCase):
    ...
    def test_score_calculation_specific(self):
        ...
        self.assertListEqual(
            list(scores["uncertainty_least_confidence"]),
            [(1 - 0.7) / (1 - 1.0 / 3.0), (1 - 0.5) / (1 - 1.0 / 3.0)],
        )
        self.assertListEqual(
            list(scores["uncertainty_margin"]), [1 - (0.7 - 0.2), 1 - (0.5 - 0.4)]
        )
        for val1, val2 in zip(
            scores["uncertainty_entropy"], _entropy(model_output) / np.log2(3)
        ):
    ...
    def test_score_calculation_binary(self):
        ...
        self.assertListEqual(
            list(scores["uncertainty_least_confidence"]),
            [(1 - 0.7) / (1 - 1.0 / 2.0), (1 - 0.6) / (1 - 1.0 / 2.0)],
        )
        self.assertListEqual(
            list(scores["uncertainty_margin"]), [1 - (0.7 - 0.3), 1 - (0.6 - 0.4)]
        )
        model_output = np.concatenate([model_output, 1 - model_output], axis=1)
        for val1, val2 in zip(
            scores["uncertainty_entropy"], _entropy(model_output) / np.log2(2)
        ):
            self.assertAlmostEqual(val1, val2, places=8)
            ...
            for num_classes in range(5):
                ...
                with self.subTest(
                    ...
                    if num_classes == 0 and num_samples > 0:
                        ...
                        scores = scorer.calculate_scores()
                        self.assertEqual(
                            set(scores.keys()), set(ScorerClassification.score_names())
                        )
