...
class SimSiamModel(BenchmarkModule):
    def __init__(self, dataloader_kNN, num_classes):
        ...
        self.resnet_simsiam = lightly.models.SimSiam(self.backbone, num_ftrs=num_ftrs)
    ...
    def configure_optimizers(self):
        optim = torch.optim.SGD(
            self.resnet_simsiam.parameters(), lr=6e-2, momentum=0.9, weight_decay=5e-4
        )
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, max_epochs)
        return [optim], [scheduler]
...
class BarlowTwinsModel(BenchmarkModule):
    def __init__(self, dataloader_kNN, num_classes):
        ...
        self.resnet_barlowtwins = lightly.models.BarlowTwins(
            self.backbone,
            num_ftrs=num_ftrs,
...
class NNBYOLModel(BenchmarkModule):
    def __init__(self, dataloader_kNN, num_classes):
        ...
        # create a byol model based on ResNet
        self.resnet_byol = lightly.models.BYOL(self.backbone, num_ftrs=num_ftrs)
        self.criterion = lightly.loss.SymNegCosineSimilarityLoss()
    ...
    def configure_optimizers(self):
        optim = torch.optim.SGD(
            self.resnet_byol.parameters(), lr=6e-2, momentum=0.9, weight_decay=5e-4
        )
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, max_epochs)
        return [optim], [scheduler]
...
class SwaVModel(BenchmarkModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        lightly.models.utils.normalize_weight(self.prototypes.layers.weight)
...
for result, model, gpu_usage in zip(bench_results, model_names, gpu_memory_usage):
    ...
    std = result_np.std()
    print(
        f"{model}: {mean:.3f} +- {std:.3f}, GPU used: {gpu_usage / (1024.0**3):.1f} GByte",
        flush=True,
    )
