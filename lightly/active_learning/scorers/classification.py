...
class ScorerClassification(Scorer):
    ...
    def ensure_valid_model_output(self, model_output: np.ndarray) -> np.ndarray:
        ...
        if len(model_output.shape) != 2:
            raise ValueError(
                "ScorerClassification model_output must be a 2-dimensional array"
            )
        if model_output.shape[1] == 0:
            raise ValueError(
                "ScorerClassification model_output must not have an empty dimension 1"
            )
    ...
    def normalize_scores_0_1(
        ...
        scores_minimum = ScorerClassification(model_output_very_sure).calculate_scores(
            normalize_to_0_1=False
        )
        scores_maximum = ScorerClassification(
            model_output_very_unsure
        ).calculate_scores(normalize_to_0_1=False)

        for score_name in scores.keys():
            interp_xp = [
                float(scores_minimum[score_name]),
                float(scores_maximum[score_name]),
            ]
            interp_fp = [0, 1]
