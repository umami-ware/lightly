...
class _UploadDatasetMixin:
    ...
    def upload_dataset(
        ...
        if len(tags) > 0:
            print(
                f"Dataset with id {self.dataset_id} has {len(tags)} tags.", flush=True
            )
        ...
        samples = self.samples_api.get_samples_by_dataset_id(dataset_id=self.dataset_id)
        ...
        total_filenames = set(dataset.get_filenames()).union(filenames_on_server_set)
        max_dataset_size = int(self.quota_api.get_quota_maximum_dataset_size())
    ...
    def _upload_single_image(
        ...
        if not metadata["is_corrupted"] and mode in ["thumbnails", "full"]:
            ...
            def upload_thumbnail(image, signed_url):
                ...
                retry(self.upload_file_with_signed_url, image_to_upload, signed_url)
